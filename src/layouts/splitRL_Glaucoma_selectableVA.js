 ...
 * |- [Navigator]
 */
export const splitRL_Glaucoma_selectableVA = {
	plot: new Map(),
	...
	buildPlotly(){
		...
		[ 'R', 'L' ].forEach(eye => {
			...
			if ( this.plot.has(`${eye}`) ){
				...
				// Procedures
				if( this.raw[`${full}Eye`].procedures ){
					addLayoutVerticals( side.get('layout'), Object.values( this.raw[`${full}Eye`].procedures ), domainLayout[1][1]);
				}

				// e.g horizontal lines with labels
				if( this.raw[`${full}Eye`].targetIOP ){
					addLayoutHorizontals( side.get('layout'), Object.values( this.raw[`${full}Eye`].targetIOP ), 'y4');
				}
